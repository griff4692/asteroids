(function () {
  ...
  Ship.prototype.redrawParticles = function (ctx) {
    var baseX = this.x + this.radialPos()[0] ;
    var baseY = this.y + this.radialPos()[1];
    var jittersX = randomFromTo(-20, 20);
    var jittersY = randomFromTo(-20, 20);

    for(var i = 0; i < this.speed() * 10; i++) {
      var particle = new Asteroids.Particle
        ({
          pos: [baseX + jittersX, baseY + jittersY],
